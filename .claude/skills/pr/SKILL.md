...
- User says "merge PR" / "/pr merge" → merge (explicit only, never automatic; an **owner-authorized routine** counts as explicit — see `/pr merge`)
...
**Routine exception:** a session running an owner-authorized routine counts as an explicit user command — see CLAUDE.md → Deployment → Routine exception. Its merges are pre-approved, including any pipeline they trigger, provided the change set is non-destructive (additive; no data migration, no history rewrite, no repo-settings change) and verification is green; the routine's own merge rules then override the pre-flight below. Destructive changes stay gated, and **release dispatch is never covered** — `workflow_dispatch` on `release.yml` always needs a separate explicit command. (ADR-0004 supersedes ADR-0003, which required an allowlisted trigger id.)
...
- **Never merge automatically.** Explicit `/pr merge` required. Exception: an owner-authorized routine, for a non-destructive change set with green verification.
