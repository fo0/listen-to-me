...
2. **This branch's pull request** — new review comments, a failed CI run, or a merge conflict. Red build →
   ...
   describe them.
3. **Verification** — the project's chain in its fixed order (there is no lint, typecheck or test framework; install
   matters on a fresh clone, which is what a cloud or routine session always is):

   ```bash
   pip install -e .                             # install — the smoke test needs PySide6 + Pillow
   npx --yes prettier@3.9.6 --check "**/*.md"   # format-check — only when Markdown changed
...
- Never add a linter, formatter or test framework to make a check pass — that is a dependency decision needing user
  sign-off (`CLAUDE.md` → _Tech Stack_).
