...
- **Floating status icon** — a small animated always-on-top icon you can drag
  ...
  as an experimental **live preview while you speak** — and you choose **where
  that text appears**: beside the icon, or **at the mouse pointer**, which the
  bubble then follows while you speak. That is the setting for dictating into a
  window on another screen, where the words otherwise show up in the corner the
  icon happens to live in. The pointer variant needs no icon at all — it works
  with the floating icon **switched off entirely**, which is arguably its most
  ...
  **monitor** you drag it to, not just a screen coordinate, so it comes back
  ...
  for when the floating icon is switched off — and with the transcript anchored
  to the mouse pointer the previews keep working with the icon off as well, so
  switching it off costs you neither the download progress nor the text.
...
- **Transcript history** — the transcribed text of each recording is kept
  ...
  search field, `Esc` clears it again without closing the window) by word
  **or by date** — a term made of digits, `-` and `:`
...
| **Assistant**    | **Connection** (shared by both profiles): API base URL, model, API key, temperature, **request timeout** (how long a dictation is held back waiting for the answer), **Test connection** (sends one sample sentence through the profile picked in the dropdown beside it and shows the reply — verifies the endpoint, that profile's model override and its prompt before a recording depends on them, with a **Cancel** button so a wrong URL or a stopped endpoint doesn't hold the page for the whole request timeout). Then one card per recording source — **Microphone dictation** and **System audio** — each with its own on/off switch and its own **system prompt** (editable, with _Reset to default…_ — it asks first once you have edited the prompt, since nothing else keeps a copy of what you wrote); the system-audio card also takes an optional **model override** (empty = the model from Connection)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **History**      | Recent **transcribed text** kept locally (never the audio), each with a **Copy** button so a lost transcript can be recovered and a **Delete** button that removes just that one entry; a **search field** (`Ctrl+F`; `Esc` clears it) narrows the list to the transcripts containing your words (any order, case-insensitive) and shows how many of them matched — a term made only of digits, `-` and `:` (e.g. `2026-09-05`, `14:`) also matches the date and time on each row; **Export…** writes the listed transcripts (the search filter applies) to a text file and **Copy all** puts the same set on the clipboard as one block; toggle history on/off, how many entries to keep, and **Clear history**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `overlay.preview_anchor`                        | `"icon"`                      | Where **both** previews above are drawn. `"icon"` = beside the floating icon, wherever you dragged it; `"cursor"` = below-right of the mouse pointer, on whichever monitor it is on, following it while you speak — and it works with the icon switched off. Any other value degrades to `"icon"`                                                                                                                           |
...
comes back. The dropdown beside the button picks **which profile** it goes
through — the connection is shared between the two, the model override and the
system prompt are not, so a green test of _Microphone dictation_ says nothing
about the model a meeting would actually be sent to. Use it before you rely on
the assistant: it runs after a dictation, so a wrong URL, an unknown model name
or a stopped Ollama would otherwise only show up once you have already spoken —
for system audio, after a recording that may have run for an hour. The status
line names the profile it tested. The reply is shown, not just an "OK" — an
...
error you are seeing to jump straight to the matching section, and step through
the other matches with `F3` / `Shift+F3` without leaving the text.
