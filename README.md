...
- **A second hotkey records what the computer plays** — a call, a meeting, a
  ...
  from the tray menu and the floating icon's menu without one. Needs a
  loopback device the system provides — see
...
- **A silent take no longer inserts a sentence nobody spoke** — press the
  ...
  speech is usually subtitled with the clip's closing line. A transcript that
  is **nothing but** such a phrase is now thrown away and reported like a take
  with no speech — while the same words inside a real dictation are never
  touched. The phrase list is yours to edit (Settings → Engine → **Silent
  takes**), see
...
| **Audio**        | **Microphone:** device selection, **Test microphone** (3-second check with a live level bar, a clear verdict — works / too quiet / no signal — and a **Cancel** button), maximum recording length. **System audio:** the hotkey for recording what the computer plays (**empty = this source is off**), its hotkey mode, the loopback **device** (“Automatic” picks the best candidate found, **Refresh** re-scans after you enable Stereo Mix or install a virtual cable) and its own maximum recording length — plus a visible hint when the machine has no loopback device at all, naming what to do about it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `filler_filter`                                 | `true`                        | Drop a transcript that is **nothing but** a phrase Whisper invents for a take with no speech — see [Silent takes and invented phrases](#silent-takes-and-invented-phrases)   |
| `filler_phrases`                                | _(18 built-in phrases)_       | The list those transcripts are matched against, one phrase per line (`#` comments allowed); editable in Settings → Engine                                                    |
...
- A transcript with **no letter or digit** in it at all (`...`, `♪♪`, a lone
  dash) is dropped too, whatever the list says — there is nothing there to
  insert either way.
- A dropped take is reported exactly like a recording with no speech: nothing
  is inserted, and nothing goes into the history.
...
#### You need a loopback device — what that means per platform

The app records from an ordinary **input** device that carries the computer's
output. Most systems have one, or can get one; Windows usually has one but
...
| **Windows** | Enable “Stereo Mix” in the Windows sound control panel (Recording tab → right-click → **Show Disabled Devices**), or install a virtual audio cable such as [VB-CABLE](https://vb-audio.com/Cable/) and play through it. |
...
The **Device** dropdown lists the devices that look like a loopback capture
first, each with the part of its name that identifies it, and every other input
device after them in case yours is named differently. **Automatic** (the
default) picks the best candidate found. **Refresh** re-scans — use it right
after enabling Stereo Mix or installing a cable, no restart needed.
...
| Platform    | Status                                                                                                                                                                                                                                                                                                                     |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Windows** | Primary target; exe built by CI. Autostart via registry `Run` key. Recording system audio needs “Stereo Mix” enabled or a virtual audio cable installed — the device exists on most onboard audio but ships disabled.                                                                                                      |
