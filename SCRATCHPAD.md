...
- **`claude/intelligent-hawking-5p60x9` (2026-09-10) — two features, docs pass done, not yet a PR.** #190 filler filter (`fillers.py`: a transcript that is _only_ a listed phrase is dropped and reported as a silent take; keys `filler_filter`/`filler_phrases`, 18 defaults, Engine page) and #191 second recording source (`system_audio.py` + `resample.py`: a second hotkey records what the computer plays through a loopback/monitor input, own device/cap/assistant profile; keys `system_audio.*`, `assistant.system_audio.*`). New **ADR-0009** (why a loopback _device_ and not a WASAPI loopback stream). Follow-ups filed as BACKLOG #39–#43. Open decision for the maintainer: **issue #192** — how to close the Windows loopback gap (ship a newer PortAudio DLL / wait for the sounddevice wheel / a `soundcard` extra).
