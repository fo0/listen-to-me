...
- **`claude/intelligent-hawking-5p60x9` (2026-09-10) — two features, PR #193 open, CI green.** #190 filler filter (`fillers.py`: a transcript that is _only_ a listed phrase is dropped, and only when the take's own audio was silent or too quiet — `clip_stats`' verdict; a take with real speech is never filtered, however it reads; keys `filler_filter`/`filler_phrases`, 18 defaults, Engine page) and #191 second recording source (`system_audio.py` + `resample.py`: a second hotkey records what the computer plays through a loopback/monitor input, own device/cap/assistant profile; keys `system_audio.*`, `assistant.system_audio.*`). New **ADR-0009** (why a loopback _device_ and not a WASAPI loopback stream). Three reviews folded in (`88b807b`, not pushed yet — PR head is `a9e36be`, and the PR body still states the old, stricter filler rule): the audio gate above (P0 — a dictated "Vielen Dank." was discarded whole), a stale loopback device index now refused instead of recording a microphone as the computer's audio, and the mute integrations no longer firing for a system-audio take. Follow-ups filed as BACKLOG #39–#48. Open decision for the maintainer: **issue #192** — how to close the Windows loopback gap (ship a newer PortAudio DLL / wait for the sounddevice wheel / a `soundcard` extra).
