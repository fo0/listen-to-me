...
- **Security boundaries stay explicit.** The updater forces TLS verification regardless of the `insecure_ssl` switch (ADR-0002), and merge pre-approval for routines is allowlist-gated rather than prompt-declared (ADR-0003). Both are deliberately narrower than the surrounding convenience feature.
