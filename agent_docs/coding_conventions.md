...
Offloaded from `CLAUDE.md` (context budget). CLAUDE.md keeps the few rules that bite most often; this file is the complete set.
...
**Silent failure is the anti-pattern that has bitten this codebase most** (autostart, tray icon, overlay) — read a write back instead of assuming it worked (`autostart.enable`, `Config.save`), and never leave a failure as a no-op the user cannot see.
