...
> **Tier-1 hooks** (already in `.claude/settings.json`): the **context budget guard** (one `PostToolUse` hook covering `CLAUDE.md` 14k / `MEMORY.md` 16k / `SCRATCHPAD.md` 8k) and the SessionStart memory reminder. There is no quality-config guard: this repo has no dedicated linter/formatter/typechecker config file to protect (Prettier runs with defaults, Python has no tooling by design), and a guard with an empty path list would be inert.
...
> **MEMORY.md / SCRATCHPAD.md size warnings are no longer here** — they are part of the Tier-1 **context budget guard** in `.claude/settings.json`, which checks all three budgeted files in a single `PostToolUse` hook and points at `agent_docs/context_budget.md`. Nothing to paste. The thresholds are the optimizer's budget table: a re-run of the optimizer replaces a hand-edited one.
...
```json
{
  ...
  "hooks": [
    {
      "type": "command",
      "command": "cmd=$(jq -r '.tool_input.command // empty'); if echo \"$cmd\" | grep -qE 'pip install [a-zA-Z]'; then printf '%s' '{\"hookSpecificOutput\":{\"hookEventName\":\"PreToolUse\",\"permissionDecision\":\"ask\",\"permissionDecisionReason\":\"New dependency — confirm with the user per the dependencies line in CLAUDE.md → Git Conventions, and keep requirements.txt + pyproject.toml (+ PyInstaller --collect-all) in sync.\"}}'; fi; exit 0"
    }
  ]
}
```

Trigger: `PreToolUse`. Forces the permission prompt instead of only mentioning the rule, so the install cannot slip through unconfirmed. An install that names no package (`pip install -e .`, `pip install -r requirements.txt`) does not match — those are the check chain's own first stage.
