...
Adding those flags still does not make the local build the released one: `release.yml` also builds PortAudio at the pinned SHA and bundles it with `--add-binary "$PORTAUDIO_DLL;."`, where `PORTAUDIO_DLL` is the built `portaudio/out/bin/portaudio.dll` (#194, ADR-0010; build and bump procedure in `agent_docs/deployment.md`). The command above ships no such DLL, so the exe it produces enumerates no `… [Loopback]` inputs and system audio behaves exactly as it does for a source install. Debugging that path needs the DLL built and passed too.
