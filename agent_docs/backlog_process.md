...
1. **Backlog is memory, not a task queue** — items are ONLY worked on upon explicit user request. Never work through the backlog independently. A user-started `/loop` is such a request, and `.claude/loop.md` bounds it: the top item, only when small and self-contained.
