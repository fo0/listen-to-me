...
**Routine exception.** A session running an **owner-authorized routine** counts as an explicit user command, for **both** acts above: its merges are pre-approved — _including_ any pipeline a merge triggers — and so is dispatching the release build, provided **both** conditions hold:
