...
>
> **Stamp:** re-measured 2026-09-10 19:02 UTC at the end of the `claude/overlay-cursor-anchor` branch (#196), working tree at `cefbb2a` with no uncommitted `.py` changes. That commit is the check: if `git log cefbb2a..HEAD -- src/listen_to_me/` is not empty, these figures are behind and the line above has to be re-run. Known to be behind already, and deliberately so: a concurrent change to `app.py`, `overlay.py` and `settings_ui.py` was still unwritten at 19:02 and is not counted here — a number measured from a file another commit is still growing is the thing this block forbids, so #196 owes it one more run before it merges.
>
