...
> Figures below were measured on 2026-09-10 with `wc -l src/listen_to_me/*.py | sort -rn | head`, while the #190/#191 branch was still being finished — they drift with every commit, so re-run that command instead of quoting any of them. **Measure at the end of a branch, not in its docs commit:** the `~5576` this block claimed for `selftest.py` was correct when it was written and 1405 lines short one commit later, because the very next commit on the same branch added the two features' regression guards. A number taken before a branch's last commit describes a file that no longer exists — which is the failure this paragraph exists to prevent, and it still happened.

- **`settings_ui.py` (~5405 lines)** — the clearest split target: one module per settings page (General / Engine / Audio / Overlay / Integrations / Assistant / History / Updates / Help) plus shared card/row helpers. The Home page already lives in `home_page.py`. Do it only when a change to the settings window is being blocked by the size. Tracked in `BACKLOG.md` (#1).
- **`selftest.py` (~6981 lines)** — grows with every regression guard, which is the point; split by check group only if navigating it starts to cost more than it saves.
- **`app.py` (~1956 lines)** — well over the ~500 bar, and the `_handle` dispatch did keep growing (#191 routes every recording branch by its source). Keep new behavior in the component modules, not in `App`; an extraction must keep `_handle`, `_register_hotkey` and `_toggle_hotkey_pause` callable **unbound on a stub**, which is how the headless self-test drives them. Tracked in `BACKLOG.md` (#40).
