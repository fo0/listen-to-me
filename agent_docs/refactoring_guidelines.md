...
> Figures below were measured on 2026-08-30 — they drift with every commit. Re-measure with `wc -l src/listen_to_me/*.py | sort -rn | head` instead of trusting a copied number.

- **`settings_ui.py` (~3853 lines)** — the clearest split target: one module per settings page (General / Whisper / Audio / Overlay / Integrations / Assistant / History / Updates / Help) plus shared card/row helpers. The Home page already lives in `home_page.py`. Do it only when a change to the settings window is being blocked by the size. Tracked in `BACKLOG.md`.
- **`selftest.py` (~4242 lines)** — grows with every regression guard, which is the point; split by check group only if navigating it starts to cost more than it saves.
- **`app.py` (~1279 lines)** — over the ~500 bar. Keep new behavior in the component modules, not in `App`; extract the event `_handle` dispatch only if it keeps growing.
- **`theme.py` (~622)**, **`overlay.py` (~1015)** — just over the bar, cohesive; no split planned, though `overlay.py` has now crossed 1000 lines and is worth a re-look if it keeps climbing. `voice_mic_widget.py` (~488) is approaching it: the painting is one animation and splitting it would only move the state flags away from the paths that read them — revisit if a third overlay mode is added. `home_page.py` (~519) has crossed back over the ~500 bar after dropping under it earlier; cohesive for now, but it is a candidate again rather than a closed one.
