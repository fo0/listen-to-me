...
>
> **Stamp:** re-measured 2026-09-10 19:17 UTC at the end of the `claude/overlay-cursor-anchor` branch (#196), working tree at `68b279f` — the branch's last code commit — with no uncommitted change to any file it gives a figure for. That commit is the check: if `git log 68b279f..HEAD -- src/listen_to_me/` shows a commit touching a file this block gives a figure for, these figures are behind and the line above has to be re-run.
>
...
- **`selftest.py` (10079 lines)** — grows with every regression guard, which is the point; split by check group only if navigating it starts to cost more than it saves.
