...
- **Supersedes:** ADR-NNNN (only when this decision replaces one)
...
Never delete an ADR. History matters. Set its status to `Superseded by ADR-NNNN` instead — the only edit an Accepted ADR ever gets.
