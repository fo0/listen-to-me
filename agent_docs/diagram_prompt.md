...
Re-read the diagram against the Syntax Rules above (they are the checklist — not restated here), plus the three shape constraints: every node carries a classDef, the layout is predominantly vertical, and the node count lands in 15–35. Click events use relative paths and stay out of visible labels.
