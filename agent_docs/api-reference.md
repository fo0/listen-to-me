...
| `-h`/`--help` | Print `app._usage()` (the flag list + where the config lives) and exit `0`. Imports no Qt. A function, not a module constant, so the config dir is resolved per run instead of at import time.                                                  |
...
`App.post(kind, payload)` is the thread-safe channel from any background thread to the main loop. Event kinds handled in `app.py → _handle`: `toggle`, `hotkey_press`, `hotkey_release`, `preview_text`, `flash_text`, `toggle_hotkey_pause`, `toggle_overlay`, `set_language`, `reset_overlay_position`, `cancel`, `copy_last`, `copy_text`, `auto_stop`, `stream_died`, `done`, `notify`, `progress`, `activate`, `settings`, `updates`, `help`, `open_config`, `open_log`, `factory_reset`, `quit`. This is the app's real "API surface" for wiring new triggers.

Two of them carry a **take id** as their payload: `auto_stop` and `stream_died` are posted from PortAudio's callback thread and are gated by `App._owns_take(payload)`, so an event that sat in the queue across a stop/start pair cannot end the wrong recording (a `None` payload keeps the old state-only behaviour). The rest ignore their payload except `preview_text` / `flash_text` (the text to show), `notify` (the message, or a `(message, force)` tuple), `copy_text` (the transcript to put on the clipboard, posted by the "Recent transcripts" menus in `tray.py` / `overlay.py`), `set_language` (the language code the tray's "Dictation language" menu picked — a code outside `choices.LANGUAGES` is logged and dropped, never written to the config) and `progress` (`None` to clear the indicator, or a `(label, fraction, done, total)` tuple from `App.progress`). When adding a kind, wire it into `_handle` and add it to the list above.
