...
### Backend abstraction (Intel via OpenVINO)
...
- **Single-instance lock** is a localhost socket bind (`app.py`, port 52697) — a second launch exits quietly.
...
- `settings_ui.py` (~1950 lines) far exceeds the size guideline — a candidate to split into per-page modules (General/Whisper/Audio/Overlay/Integrations/Assistant/History/Help) if it hinders a change. Refactor only when it blocks work (see `agent_docs/refactoring_guidelines.md`).
