...
def _config_defaults():
    ...
    assert DEFAULTS["overlay"]["preview_anchor"] == "icon"
...
def _overlay_preview_anchor_places_the_bubble():
    """Where the transcript bubble is drawn — the pure half of #196.

    `reposition_bubble` used to compute the position inline from the icon, so
    nothing could assert it without a window on a screen. It now goes through
    two pure functions of (anchor point, bubble size, screen rectangle), and
    that is what makes both of these assertable headlessly at every edge and
    corner:

    - the icon anchor has to produce *exactly* what it produced before this
      setting existed. The expected table below was generated by running
      `git show main:src/listen_to_me/overlay.py`'s own expression over these
      inputs — not re-derived from the new code, which would agree with a
      rewrite that quietly moved every existing install's bubble.
    - the cursor anchor has to keep the bubble inside the screen and off the
      pointer. Off the pointer because a bubble there hides what is about to
    ...
    through unchanged, so this function is the only thing between an unknown
    value and an unplaced bubble.
    """
    from PySide6.QtCore import QRect

    from listen_to_me.overlay import (
        ANCHOR_CURSOR,
        ANCHOR_ICON,
        cursor_bubble_position,
        icon_bubble_position,
        preview_anchor,
    )

    # --- the anchor value itself ---------------------------------------
    assert preview_anchor("icon") == ANCHOR_ICON
    assert preview_anchor("cursor") == ANCHOR_CURSOR
    # Nothing else, and near-misses least of all: a "Cursor" that silently
    # became the cursor would make the value list meaningless, and a value
    # this code cannot place must land on the placement every install has.
    for junk in ("mouse", "Cursor", "ICON", "", " cursor", "pointer", None, 3, True, [], {"a": 1}):
        assert preview_anchor(junk) == ANCHOR_ICON, repr(junk)

    # Explicit rectangles, not the platform's own screen: the expected
    # geometry below has to be the same on every machine that runs this.
    _GEO = QRect(0, 0, 1000, 800)
    _GEO_OFFSET = QRect(-1280, -200, 1280, 1024)  # a monitor left of / above the primary

    # --- the icon anchor: character-identical to before #196 -----------
    for geo, icon_x, icon_y, width, height, expected in (
        (_GEO, 0, 0, 320, 60, (4, 72)),
        (_GEO, 0, 0, 120, 400, (4, 72)),
        (_GEO, 0, 0, 320, 780, (4, 4)),
        (_GEO, 935, 0, 320, 60, (676, 72)),
        (_GEO, 935, 0, 120, 400, (876, 72)),
        (_GEO, 935, 0, 320, 780, (676, 4)),
        (_GEO, 0, 735, 320, 60, (4, 667)),
        (_GEO, 0, 735, 120, 400, (4, 327)),
        (_GEO, 0, 735, 320, 780, (4, 4)),
        (_GEO, 935, 735, 320, 60, (676, 667)),
        (_GEO, 935, 735, 120, 400, (876, 327)),
        (_GEO, 935, 735, 320, 780, (676, 4)),
        (_GEO, 500, 400, 320, 60, (372, 472)),
        (_GEO, 500, 400, 120, 400, (472, 4)),
        (_GEO, 500, 400, 320, 780, (372, 4)),
        (_GEO, -40, -30, 320, 60, (4, 42)),
        (_GEO, -40, -30, 120, 400, (4, 42)),
        (_GEO, -40, -30, 320, 780, (4, 4)),
        (_GEO_OFFSET, -1280, -200, 320, 60, (-1276, -128)),
        (_GEO_OFFSET, -1280, -200, 320, 780, (-1276, -128)),
        (_GEO_OFFSET, -65, -200, 320, 60, (-324, -128)),
        (_GEO_OFFSET, -65, -200, 320, 780, (-324, -128)),
        (_GEO_OFFSET, -1280, 759, 320, 60, (-1276, 691)),
        (_GEO_OFFSET, -1280, 759, 320, 780, (-1276, -29)),
        (_GEO_OFFSET, -65, 759, 320, 60, (-324, 691)),
        (_GEO_OFFSET, -65, 759, 320, 780, (-324, -29)),
        (_GEO_OFFSET, -640, 312, 320, 60, (-768, 384)),
        (_GEO_OFFSET, -640, 312, 320, 780, (-768, -196)),
        (_GEO_OFFSET, -1320, -230, 320, 60, (-1276, -158)),
        (_GEO_OFFSET, -1320, -230, 320, 780, (-1276, -158)),
    ):
        got = icon_bubble_position(icon_x, icon_y, width, height, geo)
        assert got == expected, (geo, icon_x, icon_y, width, height, got, expected)

    # --- the cursor anchor: inside the screen, off the pointer ---------
    def _placed(px, py, width, height, geo, *, clears=True):
        x, y = cursor_bubble_position(px, py, width, height, geo)
        rect = QRect(x, y, width, height)
        assert geo.contains(rect), ("off screen", px, py, width, height, geo, rect)
        if clears:
            assert not rect.contains(px, py), ("over the pointer", px, py, geo, rect)
        return rect

    for geo in (_GEO, _GEO_OFFSET):
        # Every edge and every corner, plus the middle and — load-bearing —
        # the band inside the far edges where the bubble no longer fits on the
        # preferred side but the clamp alone would still slide it back over
        # the pointer. A pointer on the very last pixel is not a synthetic
        # case either: that is where the taskbar and every maximized window's
        # close button live.
        for px in (
            geo.left(),
            geo.left() + 1,
        ...
        # Below-right of the pointer where there is room for it…
        near = cursor_bubble_position(geo.left() + 100, geo.top() + 100, 320, 60, geo)
        assert near == (geo.left() + 116, geo.top() + 122), near
        # …and flipped to the other side of it at the far corner, per axis.
        far = cursor_bubble_position(geo.right() - 10, geo.bottom() - 10, 320, 60, geo)
        assert far[0] < geo.right() - 10 and far[1] < geo.bottom() - 10, far
        # A bubble larger than half the screen in ONE dimension still clears
        # the pointer: the other axis has the room to move it out of the way.
        for px, py in ((geo.center().x(), geo.center().y()), (geo.left(), geo.bottom())):
            _placed(px, py, geo.width() * 3 // 4, 200, geo)
            _placed(px, py, 320, geo.height() * 3 // 4, geo)
        # Larger than half in BOTH dimensions with the pointer in the middle
        # is the one case the two invariants cannot both hold in — no such
        # position exists — so staying on screen wins. Pinned, because
        # letting the bubble hang off the screen instead would be a change of
        # mind rather than a fix.
        stuck = _placed(
            geo.center().x(),
            geo.center().y(),
            geo.width() * 3 // 4,
        ...
        assert stuck.contains(geo.center()), stuck
        # And larger than the whole screen starts at the readable end (the
        # top-left margin) rather than at whatever a clamp happens to yield.
...
def _overlay_preview_follows_the_pointer():
    """The live half of #196: one bubble, both anchors, and a tick that costs
    nothing while the pointer is still.

    Five ways this breaks with nothing looking wrong. `show_live` and `flash`
    are the same window, so an anchor honoured by only one of them reads as a
    bug rather than as a setting. The cursor anchor has to clamp against the
    screen the *pointer* is on, not the icon's — that is the whole point of
    it, and the icon's is a different rectangle on a mixed-scaling setup. It
    has to work with the floating icon switched off, which is the combination
    it is best at and which `_show_bubble`'s old "not while the icon is
    hidden" guard swallowed. The tracking rides App's 100 ms poll, so a still
    pointer must not reach the window at all — a reposition per tick is a
    bubble that flickers for the whole take. And the bubble must never take
    the click: at the pointer it is under the cursor by definition, so a
    preview that ate a click would eat it into the very field being dictated
    into.

    The pointer is stubbed rather than moved: the offscreen platform has one
    fake cursor and a check that drove the real one would be asserting about
    the platform plugin instead of about the placement.
    """
    _ensure_qapp()
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            def _bubble_at():
                return (overlay.bubble.x(), overlay.bubble.y())

            def _bubble_size():
                return (overlay.bubble.width(), overlay.bubble.height())

            # --- the default is still placed from the icon ----------------
            assert ocfg["preview_anchor"] == ANCHOR_ICON
            overlay.flash("a finished transcript")
            assert overlay.bubble.isVisible()
            assert _bubble_at() == icon_bubble_position(
                overlay.win.x(), overlay.win.y(), *_bubble_size(), geo
            )

            # --- the cursor anchor, on both previews ----------------------
            pointer = QPoint(geo.left() + 120, geo.top() + 90)
            overlay._cursor_pos = lambda: pointer  # noqa: E731 — see the docstring
            ocfg["preview_anchor"] = ANCHOR_CURSOR
            for show in (overlay.flash, overlay.show_live):
                show("the same bubble, wherever it is drawn")
                assert overlay.bubble.isVisible(), show.__name__
                width, height = _bubble_size()
                expected = cursor_bubble_position(
                    pointer.x(), pointer.y(), width, height, overlay._screen_geometry(pointer)
                )
                assert _bubble_at() == expected, (show.__name__, _bubble_at(), expected)
                # Not where the icon anchor would have put it — otherwise the
                # setting would satisfy every assertion above and do nothing.
                assert _bubble_at() != icon_bubble_position(
                    overlay.win.x(), overlay.win.y(), width, height, geo
                ), show.__name__

            # --- the tick moves only what moved ---------------------------
            placements = []
            real_reposition = overlay.reposition_bubble

            def _counting_reposition():
                placements.append(_bubble_at())
                real_reposition()

            overlay.reposition_bubble = _counting_reposition
            overlay.tick_cursor_preview()
            assert not placements, "a still pointer re-placed the bubble"
            was = _bubble_at()
            pointer = QPoint(geo.right() - 30, geo.bottom() - 20)
            overlay.tick_cursor_preview()
            assert len(placements) == 1, placements
            assert _bubble_at() != was, "the bubble did not follow the pointer"
            overlay.tick_cursor_preview()
            assert len(placements) == 1, "the same pointer position was placed twice"
            # The other anchor does not follow the pointer at all…
            ocfg["preview_anchor"] = ANCHOR_ICON
            pointer = QPoint(geo.left() + 1, geo.top() + 1)
            overlay.tick_cursor_preview()
            assert len(placements) == 1, "the icon anchor followed the pointer"
            # …and neither does a bubble that is down: moving a bubble window
            # that was never shown segfaults Qt outright (offscreen platform).
            ocfg["preview_anchor"] = ANCHOR_CURSOR
            overlay._hide_bubble()
            pointer = QPoint(geo.left() + 400, geo.top() + 400)
            overlay.tick_cursor_preview()
            assert len(placements) == 1, "a hidden bubble was re-placed"
            overlay.reposition_bubble = real_reposition

            # --- with the floating icon switched off entirely -------------
            ocfg["enabled"] = False
            overlay.set_visible(False)
            assert not overlay.win.isVisible()
            pointer = QPoint(geo.left() + 300, geo.top() + 250)
            overlay.flash("no icon anywhere, and still a preview")
            assert overlay.bubble.isVisible(), "the cursor anchor still needs the icon"
            width, height = _bubble_size()
            assert _bubble_at() == cursor_bubble_position(
                pointer.x(), pointer.y(), width, height, overlay._screen_geometry(pointer)
            )
            # Hiding the icon does not take a cursor-anchored bubble with it —
            # it is not attached to the icon.
            overlay.set_visible(False)
            assert overlay.bubble.isVisible()
            # The icon anchor is the opposite on both counts: a bubble beside
            # an icon that is gone is a caption for nothing.
            ocfg["preview_anchor"] = ANCHOR_ICON
            overlay.set_visible(False)
            assert not overlay.bubble.isVisible(), "a bubble was left beside a hidden icon"
            overlay.flash("nothing to caption")
            assert not overlay.bubble.isVisible()

            # --- never takes focus, never takes the click -----------------
            flags = overlay.bubble.windowFlags()
            assert flags & Qt.WindowType.WindowTransparentForInput, "the bubble eats clicks"
            assert flags & Qt.WindowType.WindowDoesNotAcceptFocus
            assert overlay.bubble.testAttribute(Qt.WidgetAttribute.WA_ShowWithoutActivating)
            assert overlay.bubble.testAttribute(Qt.WidgetAttribute.WA_TransparentForMouseEvents)
            # The icon must NOT be click-through — it is the start/stop button.
            assert not overlay.win.windowFlags() & Qt.WindowType.WindowTransparentForInput
            # always_on_top governs the bubble in both anchors, which is the
            # answer the Overlay page's hint gives — and toggling it must not
            # cost the click-through flag (Qt rebuilds the native window).
            for anchor in (ANCHOR_ICON, ANCHOR_CURSOR):
                ocfg["preview_anchor"] = anchor
                for wanted in (False, True):
                    ocfg["always_on_top"] = wanted
...
def _overlay_preview_anchor_round_trips():
    ...
    `_collect()` is the only path to disk, so a key missing from it is a
    setting that cannot be saved at all — and this one is the shape that
    loses a value silently: the dropdown offers labels while the config holds
    values. The store is exercised through `_collect` + `_merge_section`, the
    two halves `_apply_values` uses for a nested section, rather than through
    `_apply_values` itself — that one re-registers the global hotkey and
    rewrites the autostart entry, neither of which a self-test should do to
    the machine it runs on.

    The stored value is normalized on the way in as well: a hand-edited
    anchor has to show the place the bubble will actually appear, not reset
    the dropdown to its first row while the config still says something else.
    """
    from listen_to_me.overlay import ANCHOR_CURSOR, ANCHOR_ICON
    from listen_to_me.settings_ui import SettingsWindow
    from listen_to_me.theme import apply_theme
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            combo = window.preview_anchor_combo
            # Every offered row is a value the placement knows about — the
            # list comes from overlay.py so it cannot drift from it.
            assert [combo.itemData(i) for i in range(combo.count())] == [
                ANCHOR_ICON,
                ANCHOR_CURSOR,
            ]
            # The label talks about where the text appears, not about anchors.
            assert combo.currentData() == ANCHOR_ICON
            ...
            assert values["overlay"]["preview_anchor"] == ANCHOR_CURSOR
            # Part of the unsaved-changes snapshot too, or Save would not know
            # the page changed at all.
            assert values != window._saved_snapshot
            SettingsWindow._merge_section(stub.cfg["overlay"], values["overlay"])
            assert stub.cfg["overlay"]["preview_anchor"] == ANCHOR_CURSOR
            # …and it comes back out of the config onto the page.
            reopened = SettingsWindow(stub)
            try:
                assert reopened.preview_anchor_combo.currentData() == ANCHOR_CURSOR
                assert reopened.preview_anchor_combo.currentText() == "The mouse pointer"
            finally:
                reopened.deleteLater()
                app.processEvents()

            # A hand-edited value shows what will actually happen instead of
            # row 0 over a config that says something else.
            stub.cfg["overlay"]["preview_anchor"] = "mouse-pointer"
            hand_edited = SettingsWindow(stub)
            try:
                assert hand_edited.preview_anchor_combo.currentData() == ANCHOR_ICON
                assert hand_edited._collect()["overlay"]["preview_anchor"] == ANCHOR_ICON
            finally:
                hand_edited.deleteLater()
                app.processEvents()
        finally:
            window.deleteLater()
            app.processEvents()
